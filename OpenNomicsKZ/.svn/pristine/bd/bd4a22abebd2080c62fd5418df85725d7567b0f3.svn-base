/* 변수선언 */
var errReport = null;
var crsrPosition = new Array(); // 커서 위치
var projectGroup = new Object();
var clipboard = new Array();
var lid; // 설정 레이어
// Proj4js.defs["USER:100000"] = "+proj=tmerc +lat_0=38 +lon_0=127.5 +k=0.9996
...
// 레이어 스타일 설정정보 적용(OK버튼)
function applyLayerStyle() {
	// alert(map.get("nowStyleEditingId"));
	// 현재 편집중인 레이어 객체를 맵객체의 nowStyleEditingId키를 통해 획득
	var selectedLayer = getLayerById(map.get("nowStyleEditingId"));
	...
	// 레이어 이름을 변경한다
	selectedLayer.set("name", $("#layerName").val());
	...
	if (layerType === "point" || layerType === "linestring" || layerType === "polygon" || layerType === "multipoint"
		...
		selectedLayer.setStyle(new ol.style.Style({
			stroke : new ol.style.Stroke({
				color : rgbaStringStroke,
				width : width
			}),
		...
		selectedLayer.setSource(source);
	}
										...
										if (selectedFeatures.getLength() > 0) {
											...
											if (selectedFeatures.getLength() === 1) {
												...
												if (flag) {
													...
													// updateModifyInteraction2(collFeatures);
													...
													var theFeature = getFeatureById2(layerId, featureId);
													collFeatures.push(theFeature);
													updateSelectInteraction2(null, null);
													updateSelectInteraction2(layers, collFeatures);
													// updateModifyInteraction2(collFeatures);
...
// 10진수 rgb코드를 16진수 rgb코드로 변환하는 함수
...
// 레이어 삭제
// *************************************************************************
// Remove layers that user selected
// Parameter
// layer: Layers that will be removed - Array(ol.layer.*)
// author: YJ.SO
// *************************************************************************
...
// ol color 객체 만들기
// *************************************************************************
// Make color code for ol3
// Parameter
// r: r from RGB - string
// g: g from RGB - string
// b: b from RGB - string
// a: alpha value of color - string
// Return type: ol.Color
// author: YJ.SO
// *************************************************************************
...
// ol style 객체 만들기
// *************************************************************************
// Create layer style for ol3
// Parameter
// type: layer type ex) point, linestring, polygon - string
// colorStroke: stroke color - ol.Color
// colorFill: fill color - ol.Color
// width: width of feature style - string
// radius: radius of point - string
// Return type: ol.Style.*
// author: YJ.SO
// *************************************************************************
...
// 레이어 생성창에서 유저가 선택한 값을 추출
// *************************************************************************
// Get the style that user selected
// Return type: layer type, layer style - array<string, ol.style.*>
// author: YJ.SO
// *************************************************************************
...
// Create a tile layer
// Parameter
// baseType: base layer type - string
// Return type: ol.layer.Tile
// *************************************************************************
...
// Create a wfs layer
// Parameter
// url: layer location in geoserver - string
// name: layer name in geoserver - string
// style: layer style for openlayers3 - ol.style.*
// Return type: ol.layer.Vector
// author: YJ.SO
// *************************************************************************
...
// Parameter
// layers: layers to put into a group - array
...
function createGroupLayer(layers) {
	...
	if (layers.length >= 2) {
		...
		alert("choose two or more layers");
	}
}

// *************************************************************************
// insert a layer into a group layer
// Parameter
// group: The group layer to include layers - array
// layers: layers to be put into a group - array
// author: YJ.SO
// *************************************************************************
...
// Paremeter
// layer: a layer that properties will be inserted - ol.layer.*
// name: layer's name - string
// id: layer's id - string
// type: layer's type - string
// from: where the layer came from ex) new, file, db - string
// keyChain: The attribute names of layer - array<string>
// typeChain: The types of the attribute name - array<string>
// Return type: ol.layer.*
// *************************************************************************
			...
			buttons : {
				"확인" : function() {
					...
					case 1:
						...
						break;
					case 2:
						fileUploadAjax("/upload/upShpFile.ajax");
						break;
					default:
						break;
					}
...
function addSnapInteraction() {
	source = getAllFeatures(getAllVectorLayers());
...
function updateSnapInteraction() {
...
function updateSelectInteraction() {
...
function updateSelectInteraction2(layers, features) {
	map.removeInteraction(select);
...
// 변경
var modify;
function addModifyInteraction(features) {
	...
	// var allSource = getAllFeatures(getAllVectorLayers());
	// snap = new ol.interaction.Snap({
	// source : allSource
	// });
	// map.addInteraction(snap);
...
function updateModifyInteraction() {
	map.removeInteraction(modify);
	addModifyInteraction(select.getFeatures());
}

function updateModifyInteraction2(features) {
	map.removeInteraction(modify);
	addModifyInteraction(features);
}

// 이동
var translate;
function addTranslateInteraction(features) {
...
function removeSelectedFeature(layer, features) {
	...
	if (features.getLength() > 0) {
		var source = layer.getSource();
		// alert(nowSelectedLayers[i].get("name"));
		...
		alert("can not delete features of multiple layers");
		return;
	}
...
function insertInfoToFeature(feature, keys, values) {
	...
	if (keys.length === values.length && selectedFeatures.getLength() === 1) {
		...
		alert("key number and value number don't match");
	}
}
	...
	for (var i = 0; i < layers.length; i++) {
		...
		if (layers[i] instanceof ol.layer.Vector) {
			...
			if (layers[i].get("cat") === 0
					&& (layers[i].get("qaOption") === null || layers[i].get("qaOption") === undefined)) {
				alert("There is no QA option.");
				return;
			}
...
function approvalRequest() {
	...
	var layer = new ol.format.GeoJSON().writeFeatures(getLayerById(qaLayerNames[0]).getSource().getFeatures(), {
		dataProjection : "EPSG:4326",
		featureProjection : "EPSG:3857"
	});
...
function fileUploadAjax(url) {
	...
	$.ajax({
		...
		processData : false,
		contentType : false,
		// beforeSend : function() { // 호출전
		// $("#loadimage").position({
		// my : "center center",
		// at : "center center",
		// of : document
		// });
		// $("#loadimage").showProgress();
		// },
		success : function(data, textStatus, jqXHR) {
			// alert("success");
			// alert(data);
...
function setLayerList(div) {
	...
	layer += '</tr>';
	layer += '<tr>';
	layer += '<td colspan="2"><div id="iconTab"><span><input id="marker1" type="radio" name="markerType" value="map-marker.png" checked="checked" /><img class="markerImg" src="'+getContextPath()+'/js/gitbuilder/image/marker/map-marker.png"></span></div></td>';
	layer += '</tr>';
	layer += '</table>';
	layer += '</div>';
	...
	layer += '<form id="shpFormat" method="post" enctype="multipart/form-data" action="upload/upShpFile.ajax">';
...
function setToolBox() {
	...
	$("#mtlsDraw").click(function() {
		...
		removeMyInteraction();
		// 라인 드로우 인터랙션을 추가
		addDrawInteraction("MultiLineString");
		addSnapInteraction();
	});
	...
	$("#mtpgDraw").click(function() {
		...
		removeMyInteraction();
		// 폴리곤 드로우 인터랙션을 추가
		addDrawInteraction("MultiPolygon");
		addSnapInteraction();
	});
	...
	$("#move").click(function() {
		...
		map.removeInteraction(translate);
		// 트랜슬레잇 인터랙션을 추가
		addTranslateInteraction(select.getFeatures());
	});
	...
	$(document).on("input", ".Weight", function() {
		if(parseInt($(this).val()) === 0){
			alert("please insert value over 0");
		}
		setTotalWeight();
	});

	$(document).on("mouseenter", ".layerPropBtn", function() {
		var layer = getLayerById($(this).parent().attr("id"));
		
		if (layer instanceof ol.layer.Vector || layer instanceof ol.layer.Tile) {
			$("#layerSettingPopup").dialog("widget").find(".ui-dialog-content").css("padding", 0).css("margin", 0).css("overflow", "visible");
			$("#layerSettingPopup").dialog("widget").css("background-color", "rgba(0,0,0,0)").css("border", "0").css("overflow", "visible");
		...
		$("#attrSetting").dialog({
			...
			buttons : {
				"확인" : function() {
					...
					// alert(JSON.stringify(layer.get("attribute")));
					$(this).dialog("close");
				},
				"취소" : function() {
					$(this).dialog("close");
			...
			case "linestring":
				...
				if (qaObj.ConOverDegree) {
					qaOpt += "<tr><td><input type='checkbox' id='cnvrdgr' class='qaOptItem'  value='ConOverDegree' checked='checked'></td><td><label for='cnvrdgr'>Con over degree</label></td><td><input type='number' id='degreeValue' value='"
							+ qaObj.ConOverDegree.weight
							+ "' class='QaDegree' value='90' style='width:50px;' />degree</td><td><input type='number' id='cnvrdgrWeight' class='Weight' style='width:50px;'  min='1' max='100'>%</td></tr>";
...
function checkZoomGetFeature(apprvlLyr, lName) {
	...
	// alert(zoomLevel);
	if (zoomLevel >= 18) {
		// alert(apprvlLyr instanceof ol.layer.Vector);
		if (apprvlLyr instanceof ol.layer.Vector) {
			// alert("getin");
			var xtnt = map.getView().calculateExtent(map.getSize());
			var trnsXtnt = ol.proj.transformExtent(xtnt, "EPSG:3857", "EPSG:4326");
...
function showAttributeForFix(layer) {
	...
	if (attr === null || attr === undefined) {
		alert("Layer doesn't have attribute");
		return;
	}
	...
	$("#qaSetting").dialog({
		...
		buttons : {
			"확인" : function() {
				...
				// alert(JSON.stringify(layer.get("qaOption")));
			},
			"취소" : function() {
				$(this).dialog("close");
			}
